...
class AutoCatcher {
  ...
  catcher() {
    this.client.on("messageCreate", async (message) => {
      ...
      // Poketwo catching/AI logic (unchanged, copy your original code here)
      if (
        message.author.id === poketwo ||
        message.author.id === this.client.user.id
      ) {
        // ... Your catching, hint, AI, stats, etc logic here ...
        // Example: AI catching block, rarity checks, stats update, etc.
      }
    });

    // Fallback/manual solved channel detection
    this.client.on("messageCreate", async (message) => {
      if (
